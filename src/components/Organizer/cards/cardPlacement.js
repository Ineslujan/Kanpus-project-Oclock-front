...
export const defineCardWidth = (dateStartDay, dateStartTime, dateEndDay, dateEndTime, settings) => {
    ...
    // Is the start Date is in the morning or afternoon : 
    // false = AM ; true = PM ; to use in result
    const isStartDateInPM = defineMorningOrAfternoon(dateStartTime, "start", settings);

    // Is the end Date is in the morning or afternoon : 
    // end date : false = AM ; true = PM ; to use in result
    const isEndDateInPM = defineMorningOrAfternoon(dateEndTime, "end", settings);
    ...
    return wDiff + (diffDays === 0 ?
        (
            isEndDateInPM ?
                (
                    (isStartDateInPM && isEndDateInPM) ? 1 : 2
                ) : 1
        ) : (
            (isStartDateInPM ? 1 : 2) + (isEndDateInPM ? 2 : 1)
        )
    );
}
...
export const calculateXPos = (dayNumber, eventHour, setting) => {

    // ajustX returns 0 for before noon and 1 after noon
